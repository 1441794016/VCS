...
def create_order_one_episode(episode_limit, order_number, order_dataset):
    ...
    # orders_every_time_slot_and_region = {}
    # orders_number_every_time_slot_and_region = {}
    # for i in range(region_n):
    #     orders_every_time_slot_and_region[i] = []
    #     orders_number_every_time_slot_and_region[i] = np.zeros(episode_limit + 1, )
    #     for j in range(episode_limit + 1):
    #         orders_every_time_slot_and_region[i].append([])
    ...
    for data in order_dataset:
        ...
        if init_location[0] <= -74.00 or init_location[0] >= -73.86 or \
            ...
            init_region_location = int((init_location[0] + 74.00) / 0.01) + \
                                   int((40.80 - init_location[1]) / 0.01) * 14
            drop_off_region_location = int((drop_off_location[0] + 74.00) / 0.01) + \
                                       int((40.80 - drop_off_location[1]) / 0.01) * 14
...
class Env:
    def __init__(self, args):
        ...
        # 设置状态和动作维度
        self.observation_space = 25  # 每个车的观测状态维度
        self.action_space = 11  # 每个车动作维度。包括接单，采数据，或待在原区域不进行行动
        self.region_observation_space = 4  # 每个区域的观测维度
        self.order_observation_space = 6  # 每个order的观测维度
        self.POI_observation_space = 4  # 每个POI的观测维度
        #
    ...
    def init_POI_data(self):
        ...
        :return: no return
        """
        number_of_POIs = 70  # time slot为0时先按一个概率随机初始化一些POI在区域内
        ...
        file_path = 'dataset/dis/diff_dis.npy'
        prob = np.load(file_path).T.flatten().tolist()
    ...
    def init_POI_data_(self):
        ...
        file_path = 'dataset/dis/same_dis.npy'
        prob = np.load(file_path).T.flatten().tolist()
        region = np.random.choice(self.region_item, size=number_of_POIs, replace=True, p=prob)  # 按概率选择区域
    ...
    def init_orders(self):
        ...
        :return: no return
        """
        number_of_orders = self.first_inti_orders_number  # agent为60的时候设置为70 agentnumbe 为100 的时候是=self.first_inti_orders_number agent 为140的时候设置为140
        ...
        prob = [
            0.0240, 0.0200, 0.0000, 0.0000, 0.0028, 0.0020, 0.0, 0.0, 0.0000, 0.0000, 0.0000, 0.0000, 0.0090, 0.0110,
            0.0201, 0.0240, 0.0000, 0.0000, 0.0030, 0.0022, 0.0, 0.0, 0.0000, 0.0000, 0.0000, 0.0000, 0.0095, 0.0105,
            0.0150, 0.0150, 0.0090, 0.0050, 0.0270, 0.0250, 0.0, 0.0, 0.0000, 0.0000, 0.0060, 0.0050, 0.0300, 0.0200,
            0.0130, 0.0170, 0.0010, 0.0060, 0.0250, 0.0230, 0.0, 0.0, 0.0000, 0.0000, 0.0050, 0.0040, 0.0330, 0.0170,
            0.0270, 0.0240, 0.0000, 0.0000, 0.0075, 0.0075, 0.0, 0.0, 0.0000, 0.0000, 0.0000, 0.0000, 0.0075, 0.0075,
            0.0259, 0.0240, 0.0000, 0.0000, 0.0070, 0.0080, 0.0, 0.0, 0.0000, 0.0000, 0.0000, 0.0000, 0.0075, 0.0075,
            0.0075, 0.0075, 0.0025, 0.0025, 0.0000, 0.0000, 0.0, 0.0, 0.0045, 0.0053, 0.0375, 0.0375, 0.0100, 0.0100,
            0.0075, 0.0075, 0.0025, 0.0025, 0.0000, 0.0000, 0.0, 0.0, 0.0047, 0.0055, 0.0415, 0.0335, 0.0100, 0.0100,
            0.0120, 0.0180, 0.0000, 0.0000, 0.0000, 0.0000, 0.0, 0.0, 0.0145, 0.0105, 0.0000, 0.0000, 0.0080, 0.0100,
            0.0180, 0.0120, 0.0000, 0.0000, 0.0000, 0.0000, 0.0, 0.0, 0.0115, 0.0135, 0.0000, 0.0000, 0.0100, 0.0120
        ...
        for i, data in enumerate(region):
            self.orders.append(
                Order(order_id=self.order_number, init_location=data,
                      generate_time=self.time_slot, destination=destination[i], waiting_time=12,
    ...
    def get_observation(self):
        """
        返回agent的状态
        :return: 一个numpy数组，维度是(self.agent_n, observation_space) 是车辆的observation
                一个numpy数组， (self.region_n, region_observation_space) 是区域的observaton
                一个numpy数组， (self.region_n * 10, order_observation_space) 是order的observation
                一个numpy数组， (self.region_n * 10, POI_observation_space) 是POI的observation
        """
        ...
        obs_region = np.zeros((self.region_n, self.region_observation_space))
        obs_order = np.zeros((self.region_n * 10, self.order_observation_space))
        obs_POI = np.zeros((self.region_n * 10, self.POI_observation_space))
        ...
        for i in range(self.agent_n):
            ...
            obs[i, 14] = int(location[6])
            obs[i, 15] = int(location[7])

            obs[i, 16] = int(time_slot[0])
            obs[i, 17] = int(time_slot[1])
            obs[i, 18] = int(time_slot[2])
            obs[i, 19] = int(time_slot[3])
            obs[i, 20] = int(time_slot[4])
            obs[i, 21] = int(time_slot[5])
            obs[i, 22] = int(time_slot[6])
            obs[i, 23] = int(time_slot[7])
            obs[i, 24] = self.time_slot

        for i in range(self.region_n):
            obs_region[i, 0] = POIs[i]
            obs_region[i, 1] = idle_orders[i]
            obs_region[i, 2] = idle_vehicles[i]
            obs_region[i, 3] = i  # region 的编号
            ...
            for order in self.orders:
                if num_order >= 10:
                    break
                if not order.is_matched and not order.is_overdue and order.location == i:
                    obs_order[10 * i + num_order, 0] = order.price
                    obs_order[10 * i + num_order, 1] = order.generate_time
                    obs_order[10 * i + num_order, 2] = order.location
                    obs_order[10 * i + num_order, 3] = order.destination
                    obs_order[10 * i + num_order, 4] = order.travel_time
                    obs_order[10 * i + num_order, 5] = self.time_slot
                    num_order += 1
            for POI_ in self.POIs:
                if num_POI >= 10:
                    break
                if not POI_.is_collected and not POI_.is_overdue and POI_.location == i:
                    obs_POI[10 * i + num_POI, 0] = POI_.AOI
                    obs_POI[10 * i + num_POI, 1] = POI_.data_vol
                    obs_POI[10 * i + num_POI, 2] = POI_.location
                    obs_POI[10 * i + num_POI, 3] = self.time_slot

        return obs, obs_region, obs_order, obs_POI
    ...
    def get_graph(self):
        ...
        :return:
        """
        region2region_src = self.u
        region2region_dst = self.v
        driver2driver_src = np.arange(self.agent_n)
        driver2driver_dst = np.arange(self.agent_n)
        driver2region_src = np.arange(self.agent_n)
        driver2region_dst = np.zeros(self.agent_n, )
        for i in range(self.agent_n):
            driver2region_dst[i] = self.drivers[i].now_location
        order2region_src = np.arange(self.region_n * 10)
        order2region_dst = np.repeat(np.arange(self.region_n), 10)
        POI2region_src = np.arange(self.region_n * 10)
        POI2region_dst = np.repeat(np.arange(self.region_n), 10)

        hetero_graph = dgl.heterograph({
            ('region', 'region2region', 'region'): (region2region_src, region2region_dst),
            ('driver', 'driver2driver', 'driver'): (driver2driver_src, driver2driver_dst),
            ('driver', 'driver2region', 'region'): (driver2region_src, driver2region_dst),
            ('region', 'region2driver', 'driver'): (driver2region_dst, driver2region_src),
            ('order', 'order2region', 'region'): (order2region_src, order2region_dst),
            ('POI', 'POI2region', 'region'): (POI2region_src, POI2region_dst)
            })
        return hetero_graph
    ...
    def reset(self):
        ...
        obs_vehicle, obs_region, obs_order, obs_POI = self.get_observation()
        return obs_vehicle, obs_region, obs_order, obs_POI
    ...
    def take_action(self, action):
        ...
        #                 not self.drivers[i].is_serving and \
        #                 not self.drivers[i].is_offloading:
        #             self.drivers[i].stay = True
        #     elif driver_action == 1:
        #         # 1 表示匹配订单。优先和快要距离过期时间最近的订单进行匹配
        #         if not self.drivers[i].is_collecting and \
        #                 not self.drivers[i].is_serving and \
        #                 not self.drivers[i].is_offloading:
        #             driver_location = self.drivers[i].now_location
        ...
        #                 not self.drivers[i].is_serving and \
        #                 not self.drivers[i].is_offloading:
        #             self.drivers[i].random_walk = True
        return reward
    ...
    def step(self, action):
        ...
        obs_vehicle_next, obs_region_next, obs_order_next, obs_POI_next = self.get_observation()

        if self.time_slot <= self.episode_limit - 1:
            done_n = np.zeros(self.agent_n)
        else:
            # accepted_order_number = 0
            # overdue_order_number = 0
            # income = 0
            # accepted_POI_number = 0
            # for order in self.orders:
            #     if order.is_matched:
            #         accepted_order_number += 1
            #         income += order.price
            #     if order.is_overdue:
            #         overdue_order_number += 1
            # for POI in self.POIs:
            #     if POI.is_collected:
            #         accepted_POI_number += 1
            # print("order number:", len(self.orders))
            # print("acctpe order:", accepted_order_number)
            # print("overdue order:", overdue_order_number)
            # print("income:", income)
            # print("accept POI:", accepted_POI_number)
            done_n = np.ones(self.agent_n)

        information = None

        return obs_vehicle_next, obs_region_next, obs_order_next, obs_POI_next, reward, done_n, information
