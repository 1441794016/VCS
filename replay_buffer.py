...
class ReplayBuffer:
    ...
    def reset_buffer(self):
        self.buffer = {'obs_n': np.empty([self.batch_size, self.episode_limit, self.N, self.obs_dim]),
                       'obs_order': np.empty([self.batch_size, self.episode_limit, 1400, 6]),
                       'obs_POI': np.empty([self.batch_size, self.episode_limit, 1400, 4]),
                       'obs_region': np.empty([self.batch_size, self.episode_limit, 140, 4]),
    ...
    def store_transition(self, episode_step, obs_n, obs_order, obs_POI, obs_region, s, v_n, a_n, a_logprob_n, r_n, done_n):
        self.buffer['obs_n'][self.episode_num][episode_step] = obs_n
        self.buffer['obs_order'][self.episode_num][episode_step] = obs_order
        self.buffer['obs_POI'][self.episode_num][episode_step] = obs_POI
        self.buffer['obs_region'][self.episode_num][episode_step] = obs_region
