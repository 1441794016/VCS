...
class MAPPO:
    ...
    def choose_action(self, obs_n, evaluate):
        with torch.no_grad():
            ...
            prob = self.actor(actor_inputs.to(self.device))  # prob.shape=(N,action_dim)
    ...
    def greedy_order_action(self, obs_n):
        """
        优先接单
        :return:
        """
        ...
        for i, obs in enumerate(obs_n):
            if obs[5] > 0:
                if np.random.rand(1) > 0.94:
                    action[i] = 0
                else:
                    action[i] = np.random.randint(0, self.action_dim, 1)[0]
            else:
                action[i] = np.random.randint(0, self.action_dim, 1)[0]
        return action

    def greedy_POI_action(self, obs_n):
        """
        优先接单
        :return:
        """
